...
class DataFrame:
    ...
    def replace_with_mean(self, column_name: str):
        self.df[column_name] = self.df[column_name].fillna(self.df[column_name].mean())

    def remove_rows_with_more_than_three_features_missing(self):
        # Count the number of null values in each row
        null_counts = self.df.isnull().sum(axis=1)
        # Identify rows with three or more null values
        rows_to_remove = null_counts[null_counts >= 3].index
        # Remove rows with three or more null values
        df_cleaned = self.df.drop(index=rows_to_remove)
        self.df = df_cleaned
